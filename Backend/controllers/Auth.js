	...
	try {
		...
		try {
			const emailResponse = await mailSender(
				...
				"Studynotion by - Harsh Patel",
				passwordUpdated(
					updatedUserDetails.email,
					updatedUserDetails.firstName
				)
			);
			console.log("Email sent successfully:", emailResponse,updatedUserDetails);
		} catch (error) {
...
exports.googlelogin = async (req, res) => {
  ...
  try {
    ...
    // Re-fetch user with populated profile
    ...
    // Send response
    return res.cookie("token", appToken, options).status(200).json({
      success: true,
      token: appToken,
      user: userObj,
