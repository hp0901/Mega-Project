...
export default function PublishCourse() {
  ...
  const { course } = useSelector((state) => state.course)
  const [loading, setLoading] = useState(false)
  const [thumbnailFile] = useState(null); // This is fine, it remains null
  ...
  const handleCoursePublish = async () => {
    ...
    // This component only sends text. We do not need FormData.
    // We will create a plain JSON object instead.
    
    // const formData = new FormData(); // <-- DELETE THIS
    // formData.append("courseId", course._id); // <-- DELETE THIS
    
    const courseStatus = getValues("public") ? COURSE_STATUS.PUBLISHED : COURSE_STATUS.DRAFT;
    // formData.append("status", courseStatus); // <-- DELETE THIS
    ...
    if (thumbnailFile) {
      // This will be false, which is correct for this component
      // formData.append("thumbnailImage", thumbnailFile); 
      console.log("Thumbnail is ", thumbnailFile)
    }
    ...
    try {
      ...
      // Pass the new JSON 'data' object instead of 'formData'
      const result = await editCourseDetails(data, token); 
      
      console.log("Data sent to API: ", data) // Log the object
      console.log("Token is ", token)
      console.log("Result is ",result)
